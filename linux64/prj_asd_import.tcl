...
proc import_assignments_from_maxplus2 { filename args } {
	...
	if { [string compare $do_clique "TRUE"] == 0 } {
		if { [string compare $cur_family "FLEX10K"] == 0 } {
			show_message info "Quartus Prime does not support clique assignments in the same manner as MAX+PLUS II for FLEX 10K/ACEX 1K devices. Will not import any clique assignments found in $entity.acf."
			set do_clique "FALSE"
		}
	}
				...
				#	set source [ string trim [ string range $source 1 [ expr [ string length $source ] - 2 ] ] ]
						...
						CLIQUE {
							if {[string compare "FALSE" [synth_text_hier_name $node]] == 0} {
								if { [string compare $cur_family "FLEX6K"] == 0} {
									...
									show_message info "Quartus Prime does not support clique assignments for MAX7000B/7000AE/3000A/MAX7000S devices. Will not import any clique assignments found in $entity.acf."
								} elseif { [string compare $cur_family "MAX7000AE"] == 0 } {
									show_message info "Quartus Prime does not support clique assignments for MAX7000B/7000AE/3000A/MAX7000S devices. Will not import any clique assignments found in $entity.acf."
								} elseif { [string compare $cur_family "MAX3000A"] == 0 } {
									show_message info "Quartus Prime does not support clique assignments for MAX7000B/7000AE/3000A/MAX7000S devices. Will not import any clique assignments found in $entity.acf."
									...
									if { [string compare "ANY" $value] != 0 } {
										if { [string first "IOC" $value] == -1 } {
											...
											if { [string compare $first_ioc_warning "TRUE"] == 0 } {
												show_message info "IOC location assignments are not supported by Quartus Prime. Will automatically translate an IOC location assignment to the Fast Input/Output Register logic assignment, but exact I/O placement will be lost."
								...
								if { [string compare $value "ON"] == 0 } {
									if { [string compare $edif_lmf_file1 ""] != 0 } {
										if { [string compare $eda_add_lmf "FALSE"] == 0 } {
											...
											show_message info "Quartus Prime can only accept one LMF file. Using the second LMF file $edif_lmf_file2, and ignoring the first LMF file $edif_lmf_file1."
										}
									}
								}
							}
								...
								if { [string compare $value "ON"] == 0 } {
									if { [string compare $edif_lmf_file2 ""] != 0 } {
										if { [string compare $eda_add_lmf "FALSE"] == 0 } {
											...
											show_message info "Quartus Prime can only accept one LMF file. Using the first LMF file $edif_lmf_file1, and ignoring the second LMF file $edif_lmf_file2."
										}
									}
								}
							}
					...
					if { [string compare $do_eda_output "TRUE"] == 0 } {
						switch -exact $variable {
							VERILOG_NETLIST_WRITER {
								if { [string compare [string toupper $value] "ON"] == 0 } {
									if { [string compare $eda_output_format ""] == 0 } {
										...
										show_message info "Quartus Prime can only generate one netlist format at one time. Selecting VHDL output."
									}
								}
							}
							...
							VHDL_NETLIST_WRITER {
								if { [string compare [string toupper $value] "ON"] == 0 } {	
									if { [string compare $eda_output_format ""] == 0 } {
										...
										show_message info "Quartus Prime can only generate one netlist format at one time. Selecting VERILOG format."
									}
								}
							}
							VHDL_WRITER_VERSION {
							...
							EDIF_NETLIST_WRITER {
								if { [string compare [string toupper $value] "ON"] == 0 } {
									show_message info "Quartus Prime cannot generate EDIF files. Only Verilog or VHDL formats can be generated."
								}
							}
						}
					}							 
					...
					if { [string compare $chip_group ""] != 0 } {
						if { [string compare $chip_group $group] != 0 } {
							if { [string compare $first_multichip_warning "TRUE"] == 0 } {
								show_message info "Multiple CHIP sections found, but Quartus Prime does not support multidevice partitioning. Will import assignments in the CHIP $chip_group section and ignore the rest of the CHIP sections."
								set do_chip "FALSE"
...
proc bus_notation_warning { do_bus_warn_msg first_node_warning array orig_node bus_notation_node } {
	...
	if { [string length $do_bus_warn_msg] != 0 } {
		if { [string compare $first_node_warning "TRUE"] == 0 } {
			...
			set msg "Due to differences in the way the Quartus Prime software interprets bus notation, conversion of your MAX+PLUS II project can create obsolete assignments. "
...
proc print_report { bus_mapping_file_name array entity ignore_node_list } {
	...
	append rt " -- Copyright (C) 1991-$current_year Altera Corporation. All rights reserved.\n"
	...
	append rt " -- associated  documentation or information  provided by  Intel  or a partner\n"
	append rt " -- under  Intel's   Megafunction   Partnership   Program  may  be  used  only\n"
	append rt " -- to program  PLD  devices (but not masked  PLD  devices) from  Intel.   Any\n"
	...
	append rt " -- a separate agreement with  Intel  or a megafunction partner.  Title to the\n"
	...
	append rt " -- related documentation or information provided by  Intel  or a megafunction\n"
	append rt " -- partner, remains with Intel, the megafunction partner, or their respective\n"
	...
	if { [array size bus_notation_array] > 0 } {
		...
		::struct::matrix mtx
		mtx add columns 2
		mtx add row [list "MAX+PLUS II node name" "Quartus Prime node name"]
